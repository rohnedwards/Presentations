...
namespace ROE.ThirdParty.Microsoft.Experimental.IO {
    ...
    public static class LongPathDirectory {
        ...
        public static void Create(string path) {
            ...
            if (!Microsoft.Experimental.IO.Interop.NativeMethods.CreateDirectory(normalizedPath, IntPtr.Zero)) {

                // To mimic Directory.CreateDirectory, we don't throw if the directory (not a file) already exists
                int errorCode = Marshal.GetLastWin32Error();
                if (errorCode != Microsoft.Experimental.IO.Interop.NativeMethods.ERROR_ALREADY_EXISTS || !LongPathDirectory.Exists(path)) {
        ...
        public static void Delete(string path) {
            ...
            if (!Microsoft.Experimental.IO.Interop.NativeMethods.RemoveDirectory(normalizedPath)) {
        ...
        private static IEnumerable<string> EnumerateFileSystemIterator(string normalizedPath, string normalizedSearchPattern, bool includeDirectories, bool includeFiles) {
            ...
            Microsoft.Experimental.IO.Interop.NativeMethods.WIN32_FIND_DATA findData;
            using (SafeFindHandle handle = BeginFind(Path.Combine(normalizedPath, normalizedSearchPattern), out findData)) {
                ...
                } while (Microsoft.Experimental.IO.Interop.NativeMethods.FindNextFile(handle, out findData));

                int errorCode = Marshal.GetLastWin32Error();
                if (errorCode != Microsoft.Experimental.IO.Interop.NativeMethods.ERROR_NO_MORE_FILES)
                    throw LongPathCommon.GetExceptionFromWin32Error(errorCode);
            }
        }

        private static SafeFindHandle BeginFind(string normalizedPathWithSearchPattern, out Microsoft.Experimental.IO.Interop.NativeMethods.WIN32_FIND_DATA findData) {
            
            SafeFindHandle handle = Microsoft.Experimental.IO.Interop.NativeMethods.FindFirstFile(normalizedPathWithSearchPattern, out findData);
            if (handle.IsInvalid) {

                int errorCode = Marshal.GetLastWin32Error();
                if (errorCode != Microsoft.Experimental.IO.Interop.NativeMethods.ERROR_FILE_NOT_FOUND)
                    throw LongPathCommon.GetExceptionFromWin32Error(errorCode);
