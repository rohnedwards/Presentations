...
#>
    ...
    $__PropertyQuoteFormat = '"{0}"'
    # $__ColumnQuoteFormat = '"{0}"'
    $__ColumnQuoteFormat = '{0}'
    ...
    $CommandAttributes = @{
        ...
        DbFormatTableInfo = 'MagicDbFormatTableInfo'
    }
    $__DefaultFormatViewName = 'Default'
    ...
    $ParameterAttributes = @{
        ...
        DbFormatTableColumnInfo = 'MagicDbFormatTableColumn'
    }
...
#
        ...
        process {
            ...
            foreach ($__Property in $__MyCommandInfo.PropertyParameters.GetEnumerator()) {
                ...
                $QuotedColumnName = $__ColumnQuoteFormat -f $__Property.Value.ColumnName
                $QuotedPropertyName = $__PropertyQuoteFormat -f $__Property.Value.PropertyName
                if (-not $PSBoundParameters.ContainsKey('GroupBy') -or (ContainsMatch -Collection $GroupBy -ValueToMatch $__Property.Value.PropertyName) -or (ContainsMatch -Collection $GroupBy -ValueToMatch $__Property.Name)) {
                    $__CurrentSelect = '{0} AS {1}' -f $QuotedColumnName, $QuotedPropertyName
                    if (-not $__StringList.Contains($__CurrentSelect)) {
                        $__StringList.Add($__CurrentSelect) | Out-Null
                    }

                    if (-not $__GroupByList.Contains($QuotedColumnName)) {
                        $__GroupByList.Add($QuotedColumnName) | Out-Null
                    }
                }
            }
            ...
            if ($PSBoundParameters.ContainsKey('GroupBy')) {
                ...
                $QuotedCountProp = $__PropertyQuoteFormat -f 'Count'
                $__StringList.Add("COUNT(*) AS ${QuotedCountProp}")
            }
...
'@ -f ($__ParamStrings -join "`n")
    ...
    # Helper functions:
    function New-FormatTableInfo {
        <#
        Helper function that allows a hash table from the FakeAttributes format to be passed in. Defaults (like if a ViewName isn't specified) are taken
        care of by using a helper function this way. This can also provide warning messages when non-supported properites are used in the attribute list...

        Reference: https://docs.microsoft.com/en-us/powershell/developer/format/creating-a-table-view
        #>
        ...
        param(
            [Parameter(Mandatory)]
            [string] $__PSTypeName,
            [string] $View = $__DefaultFormatViewName,
            [Parameter()]
            [AllowNull()]
            ...
            [Parameter()]
            [AllowNull()]
            [scriptblock] $GroupByScriptBlock,
            [Parameter()]
            [AllowNull()]
            [AllowEmptyString()]
            [string[]] $__UnnamedArgs,
        ...
        process {
            if ($__UnnamedArgs) {
                Write-Warning "The following unnamed arguments were provided to the Format-Table attribute for the ${Label} property: $($__UnnamedArgs -join ', ')"
            }

            if ($RemainingArgs) {
                Write-Warning "The following unknown/unsupported arguments were provided to the Format-Table attribute for the ${Label} property and are being ignored: $($RemainingArgs -join ', ')"
            }
            ...
            foreach ($CurrProp in $PSBoundParameters.Keys) {
                if ($CurrProp -in '__UnnamedArgs', 'RemainingArgs') {
                    continue
                }
                $ReturnObjProps[$CurrProp] = $PSBoundParameters[$CurrProp]
            }

            [PSCustomObject] $ReturnObjProps
        }
    }
    function New-FormatTableColumnInfo {
        <#
        Helper function that allows a hash table from the FakeAttributes format to be passed in. Defaults (like if a ViewName isn't specified) are taken
        care of by using a helper function this way. This can also provide warning messages when non-supported properites are used in the attribute list...
...
"@)
        ...
        process {
            foreach ($CurrentProperty in $Property) {
                if ($CurrentProperty -is [hashtable]) {
                    ...
                    $PropertyName = $__PropertyQuoteFormat -f $CurrentProperty.ToString()
    ...
    #>
        ...
        $CommandFormatInfo = New-Object System.Collections.ArrayList
        $ParamBlockAst.Attributes | Where-Object { $_.TypeName.Name -eq $CommandAttributes.DbFormatTableInfo } | ForEach-Object {
            $CurrentFormatInfo = [ordered] @{}
            foreach ($NamedArg in $_.NamedArguments) {
                $CurrentFormatInfo[$NamedArg.ArgumentName] = $NamedArg.Argument | GetValueFromAst
            }

            if (-not $DbInfo.Contains('PSTypeName')) {
                # Everything gets a PSTypeName just in case formatting is used
                $DbInfo.PSTypeName = 'DatabaseReporter-{0}-{1}' -f $CommandName, [guid]::NewGuid()
            }

            $CurrentFormatInfo.__PSTypeName = $DbInfo.PSTypeName

            $CommandFormatInfo.Add(( New-FormatTableInfo @CurrentFormatInfo )) | Out-Null

        }
        $ReturnHashtable.FormatInfo = $CommandFormatInfo
    ...
    if (-not $DontDoHelp) {          # THIS IS FOR DEBUGGING PURPOSES FOR NOW
            ...
            switch ($token.Kind) {
                Variable {
                    ...
                    if ($CurrentComments.Length) {
                        ...
                        $null = $CurrentComments.Clear() | Out-Null
                    }
                }
                ...
                Comment {
                    ...
                    # it belongs to)
                    $null = $CurrentComments.AppendLine($token.Text) | Out-Null
                }
        ...
        foreach ($ParamAst in $ParamBlockAst.Parameters) {
            ...
            foreach ($CurrentParamTable in $ParamHashTables) {
                ...
                $null = $ParamText.Append($ParamTextAttributes)
            }
        }
    ...
    function New-FormatTableXml {
        ...
        begin {
            $CollectedObjects = New-Object System.Collections.ArrayList

            # This part belongs outside of this function so that XML can be passed into a list and table
            # XML creator, and just the <View> portion is added in the helper functions
            $Xml = New-Object xml
            $Configuration = $Xml.AppendChild($Xml.CreateElement('Configuration'))
            $ViewDefs = $Configuration.AppendChild($Xml.CreateElement('ViewDefinitions'))
        }
        process {
            foreach ($Current in $InputObject) {
                if ($Current.__FormatType -eq 'Table') {
                    $CollectedObjects.Add($Current) | Out-Null
                }
            }
        }
        end {
            $CollectedObjects | ForEach-Object { $_ } | Group-Object __PSTypeName | ForEach-Object {
                $PSTypeName = $_.Name
                $_.Group | Group-Object View | ForEach-Object {

                    $ViewName = $_.Name
                    $ViewXml = $ViewDefs.AppendChild($Xml.CreateElement('View'))
                    $ViewXml.AppendChild($Xml.CreateElement('Name')).InnerText = $ViewName
                    $ViewXml.AppendChild($Xml.CreateElement('ViewSelectedBy')).AppendChild($Xml.CreateElement('TypeName')).InnerText = $PSTypeName

                    $InfoDict = $_.Group | Group-Object __SubType -AsHashTable -AsString

                    if ($InfoDict.TableInfo.Count -gt 1) {
                        Write-Warning "More than one table definition defined for '${ViewName}' view on '${PSTypeName}' type, so using the first one"
                    }
                    $TableInfo = $InfoDict.TableInfo | Select-Object -First 1

                    $GroupByNode = $null
                    foreach ($GroupByProp in 'GroupByLabel', 'GroupByScriptBlock', 'GroupByPropertyName') {
                        if ($TableInfo.$GroupByProp) {
                            if (-not $GroupByNode) {
                                $GroupByNode = $ViewXml.AppendChild($Xml.CreateElement('GroupBy'))
                            }

                            $GroupByNode.AppendChild($Xml.CreateElement($GroupByProp -replace '^GroupBy')).InnerText = $TableInfo.$GroupByProp -as [string]
                        }
                    }

                    $TableControl = $ViewXml.AppendChild($Xml.CreateElement('TableControl'))
                    $TableHeaders = $TableControl.AppendChild($Xml.CreateElement('TableHeaders'))
                    $TableRowEntries = $TableControl.AppendChild($Xml.CreateElement('TableRowEntries')).AppendChild($Xml.CreateElement('TableRowEntry'))
                    $TableColumnItems = $TableRowEntries.AppendChild($Xml.CreateElement('TableColumnItems'))

                    if ($null -ne $TableInfo.Wrap) {
                        $TableRowEntries.AppendChild($Xml.CreateElement('Wrap')) | Out-Null
                    }

                    foreach ($RowInfo in $InfoDict.RowInfo) {
                        $CurrentHeader = $TableHeaders.AppendChild($Xml.CreateElement('TableColumnHeader'))
                        $CurrentEntry = $TableColumnItems.AppendChild($Xml.CreateElement('TableColumnItem'))

                        foreach ($HeaderProp in 'Label', 'Width', 'Alignment') {
                            if ($null -ne $RowInfo.$HeaderProp) {
                                $CurrentHeader.AppendChild($Xml.CreateElement($HeaderProp)).InnerText = $RowInfo.$HeaderProp -as [string]
                            }
                        }

                        foreach ($ColumnProp in 'PropertyName', 'ScriptBlock', 'FormatString', 'Alignment') {
                            if ($null -ne $RowInfo.$ColumnProp) {
                                $CurrentEntry.AppendChild($Xml.CreateElement($ColumnProp)).InnerText = $RowInfo.$ColumnProp -as [string]
                            }
                        }
                    }
                }
...
#            Write-Warning "need to sanitize PSTypeName"
    ...
    $CommandDbInformation.FormatInfo = $DefinitionParsedParamBlock.FormatInfo
    
    $FormatInfo = $CommandDbInformation.FormatInfo
...
Write-Debug 'Checking for fake attributes'
        if ($Parameter.Value.FakeAttributes.ContainsKey($ParameterAttributes.DbColumnProperty)) {
            ...
            foreach ($FormatTableColumnInfo in $Parameter.Value.FakeAttributes[$ParameterAttributes.DbFormatTableColumnInfo]) {
                $Type = 'Table'

                # It seems that $FormatTableColumnInfo is already a PSObject at this point :(. Let's turn it into a dictionary
                # for now, but might need to go change the thing that turns it from a dictionary into a PSObject
                $FormatTableColumnInfoDict = @{}
                foreach ($CurrProp in $FormatTableColumnInfo.psobject.Properties) {
                    $FormatTableColumnInfoDict[$CurrProp.Name] = $CurrProp.Value
                }

                if (-not ($FormatTableColumnInfoDict.Contains('PropertyName') -and $FormatTableColumnInfoDict.Contains('Expression'))) {
                    $FormatTableColumnInfoDict.PropertyName = $DbPropertyInfo.PropertyName
                }

                if (-not ($FormatTableColumnInfoDict.Contains('Label'))) {
                    $FormatTableColumnInfoDict.Label = $DbPropertyInfo.PropertyName
                }
                $FormatTableColumnInfoDict.__PSTypeName = $CommandDbInformation.PSTypeName

                $FormatInfo.Add(( & $DBRModule { $dict = $args[0]; New-FormatTableColumnInfo @dict } $FormatTableColumnInfoDict )) | Out-Null
...
#>
    ...
    # Turn collected command and parameter format information into a real formatting file:
    $PsTypeName = $CommandDbInformation.PsTypeName
    if ($PSTypeName) {

        $TempFile = "$env:temp\db_reader_${pstypename}_format_file.ps1xml"
        & $DBRModule { New-FormatTableXml -InputObject $args } $CommandDbInformation.FormatInfo | Out-File -FilePath $TempFile -Force
